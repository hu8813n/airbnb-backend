...
export async function getIdempotencyKeyData(tx:Prisma.TransactionClient, key: string) {
  const isValidUUID = validate(key);
  if(!isValidUUID) {
...
export async function confirmBooking(tx: Prisma.TransactionClient, bookingId: number | null) {
  if(!bookingId) {
